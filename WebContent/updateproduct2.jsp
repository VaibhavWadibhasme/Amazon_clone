...
<body>
	...
	<%
		...
		try {
			...
			String sql = "select * from product_details where seller_name='" + seslogseller + "' and id='" + id
					+ "'";
	...
	<div class="container" style="font-size: 18px; margin: 10px;">
		<form action="proupdateserver.jsp" method="post"
			...
			<table>
				...
				<tr>
					...
					<td><select class="" id="category" name="category">
							...
							<option
								<%if (category.equalsIgnoreCase("Beauty & Personal Care"))
						out.println("selected");%>>Beauty
								&amp; Personal Care</option>
							<option
								<%if (category.equalsIgnoreCase("Books"))
						out.println("selected");%>>Books</option>
							<option
								<%if (category.equalsIgnoreCase("Cell Phones & Accessories"))
						out.println("selected");%>>Cell
								Phones &amp; Accessories</option>
							<option
								<%if (category.equalsIgnoreCase("Clothing, Shoes & Jewelry"))
						out.println("selected");%>>Clothing,
								Shoes &amp; Jewelry</option>
							<option
								<%if (category.equalsIgnoreCase("Women"))
						out.println("selected");%>>Women</option>
							<option
								<%if (category.equalsIgnoreCase("Men"))
						out.println("selected");%>>Men</option>
							<option
								<%if (category.equalsIgnoreCase("Girls"))
						out.println("selected");%>>Girls</option>
							<option
								<%if (category.equalsIgnoreCase("Boys"))
						out.println("selected");%>>Boys</option>
							<option
								<%if (category.equalsIgnoreCase("Baby"))
						out.println("selected");%>>Baby</option>
							<option
								<%if (category.equalsIgnoreCase("Computers"))
						out.println("selected");%>>Computers</option>
							<option
								<%if (category.equalsIgnoreCase("Electronics"))
						out.println("selected");%>>Electronics</option>
							<option
								<%if (category.equalsIgnoreCase("Grocery & Gourmet Food"))
						out.println("selected");%>>Grocery
								&amp; Gourmet Food</option>
							<option
								<%if (category.equalsIgnoreCase("Health, Household & Baby Care"))
						out.println("selected");%>>Health,
								Household &amp; Baby Care</option>
							<option
								<%if (category.equalsIgnoreCase("Home & Kitchen"))
						out.println("selected");%>>Home
								&amp; Kitchen</option>
							<option
								<%if (category.equalsIgnoreCase("Luggage & Travel Gear"))
						out.println("selected");%>>Luggage
								&amp; Travel Gear</option>
					</select></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input type="text" name="quantity" value="<%=qyt%>"></td>

				</tr>
				<tr>
					<td><strong>Specification</strong></td>
					<td><textarea id="specification" name="specification" rows="7"><%=spec%></textarea></td>
				</tr>
				<tr>
					<td><strong>Product Image</strong></td>
					<td><input type="file" class="form-control-file" id="image1"
						name="image1"></td>

				</tr>
				<tr>
					<td><strong>Product Image</strong></td>
					<td><input type="file" class="form-control-file" id="image2"
						name="image2"></td>

				</tr>

				<tr>
					<td><button class="btn btn-primary" type="submit" id="amazon">Update
