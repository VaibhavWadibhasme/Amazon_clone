	...
	<section id="myproducts">
		...
		<%
			try {
				Class.forName("com.mysql.jdbc.Driver");
				Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/amazon", "root", "");
				String sql = "select * from cart where email='" + seslog + "' and status='1'";
				PreparedStatement pst = con.prepareStatement(sql);
				ResultSet rs = pst.executeQuery();
				while (rs.next()) {
					int id = rs.getInt("id");
		...
		<div class="container-fluid ">
			<div class="row">
				...
				<div class="col col-lg-6 col-md-6 col-sm-6 col-xs-6">
					...
					<p>
						Delivered by<strong style="color: green;">&nbsp;Monday,
							December 26</strong>

					</p>
					<p>FREE Delivery by Amazon</p>
					<div class="form-group">
						<label for="">Quantity : </label> &nbsp;<%=qyt%>
					</div>
					<a class="btn" href="cancle.jsp?id=<%=id%>&qyt=<%=qyt%>">Cancel</a>
				</div>
			</div>
		</div>
		<%
	...
	<section id="myproducts">
		...
		<%
			try {
				Class.forName("com.mysql.jdbc.Driver");
				Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/amazon", "root", "");
				String sql = "select * from cancle_order where email='" + seslog + "'";
				PreparedStatement pst = con.prepareStatement(sql);
				ResultSet rs = pst.executeQuery();
				while (rs.next()) {
					int id = rs.getInt("id");
		...
		<div class="container-fluid ">
			<div class="row">
				...
				<div class="col col-lg-6 col-md-6 col-sm-6 col-xs-6">
					...
					<a style="color: red;">Canceled Order</a>
				</div>
			</div>
		</div>
		<%
